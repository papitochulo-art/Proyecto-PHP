...
if (!isset($_SESSION["usuario"])) header("Location:login.php");
    ...
    if (isset($_POST["eliminar"])) {
        if (isset($_POST["lista"])) {
            ...
            foreach ($lista as $valor) {
                ...
                try {
                    ...
                    if ($resultado){
                        $cont=0;
                        $insertar="INSERT INTO usuarioseliminados(nombre,password,email) values(";
                        while($columnas=$resultado->fetch(PDO::FETCH_ASSOC)){
                            $insertar.="'".$columnas["nombre"]."','".$columnas["password"]."','".$columnas["email"]."')";
                        }
        ...
        }else header("Location:eliminarusuario.php?error='error'");
    }
?>
            ...
            <th>Eliminar</th>
                ...
                <?php
                    ...
                    while($columnas=$resultado->fetch(PDO::FETCH_ASSOC)){
                        for ($i = 0; $i < count($arrayColumnas); $i++){
                            if($i==0){ 
                                if($columnas["nombre"]!="Root")print "<tr><td><input type='checkbox' name='lista[]' value='".$columnas[$arrayColumnas[$i]]."'></td><td>".$columnas[$arrayColumnas[$i]]."</td>";
                                else print "<tr><td></td><td>".$columnas[$arrayColumnas[$i]]."</td>";
                            }
                                else print "<td>".$columnas[$arrayColumnas[$i]]."</td>";
                        }
                    }
                    if(isset($_REQUEST["error"])) print "<p style='color:red;'>No has seleccionado ningun usuario</p>";
                } catch (PDOException $e) {
                    print $e->getMessage();
                }
                ?>
