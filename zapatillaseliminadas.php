    ...
    <body>
        <div class="container-fluid">
            ...
            <table class="table table-hover table-dark">
                <?php
                $consultaColumnas = "SHOW COLUMNS FROM zapatillaseliminadas";
                $arrayColumnas = array();
                try {
                ...
                <?php
                    ...
                    $consulta="SELECT * from zapatillaseliminadas";
                    $resultado = $db->prepare($consulta);
                    $resultado->execute();
                    if($resultado->rowCount()>0){
                    while($columnas=$resultado->fetch(PDO::FETCH_ASSOC)){
                        for ($i = 0; $i < count($arrayColumnas); $i++){
                            if($i==0)print "<tr><td>".$columnas[$arrayColumnas[$i]]."</td>";
                            else if($i==count($arrayColumnas)-1) print "<td>".$columnas[$arrayColumnas[$i]]."</td></tr>";
                            else print "<td>".$columnas[$arrayColumnas[$i]]."</td>";
                        }
                        
                    }
                }else print "No hay zapatillas eliminadas";
